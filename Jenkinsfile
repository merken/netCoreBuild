...
def docker_run(){
    dir('Merken.NetCoreBuild.App') {
        ...
        def containerId = sh (script: 'curl -v -X POST -H "Content-Type:application/json" -i -H "Accept: application/json" --unix-socket /var/run/docker.sock -d @imageconf http://0.0.0.0:2375/containers/create', returnStdout: true)
        sh "echo container created: $containerId"
        sh "echo $containerId"
        def jsonSlurper = new JsonSlurper()
        def object = jsonSlurper.parseText(containerId)
        def idObject = object.id
    
        sh "echo container id: $idObject"
