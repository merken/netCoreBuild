...
def dotnet_publish(){
    dir('Merken.NetCoreBuild.App') {
        ...
        sh('echo building')
        sh(script: 'curl -v -X POST -H "Content-Type:application/tar" --data-binary netcoreapp.tar.gz --unix-socket /var/run/docker.sock http://0.0.0.0:2375?t=sample', returnStdout: true)
    }
}