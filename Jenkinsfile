...
def docker_run(){
    dir('Merken.NetCoreBuild.App') {
        ...
        def containerId = sh 'curl -v -X POST -H "Content-Type:application/json" -i -H "Accept: application/json" --unix-socket /var/run/docker.sock -d @imageconf http://0.0.0.0:2375/containers/create'
