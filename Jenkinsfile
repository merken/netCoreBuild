...
def docker_build(){
    dir('Merken.NetCoreBuild.App') {
        ...
        sh(script: 'curl -v -X POST -H "Content-Type:application/x-tar" --data-binary @netcoreapp.tar.gz --dump-header - --no-buffer --unix-socket /var/run/docker.sock http://0.0.0.0:2375/build?&t=netcoreapp&rm=1&nocache=1', returnStdout: true)
    }
}
